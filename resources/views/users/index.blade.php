...
<div class="card">
    <div class="card-header">
        ...
        <div class="card-header-right">
               <form action="{{ url('users/create')}}" method="post" style="display: inline-block">
                @csrf
                @method('GET')
...
<tr>
    ...
    <td>
     @if($user->role==1)
    <button class="btn btn-danger btn-sm"></i> Admin  </button>
    @else
    <button class="btn btn-success btn-sm"> User  </button>
    @endif
    </td>
    <td>
    @if($user->status==1)
    ...
    <!-- <td>
    <form action="{{ url('users/edit', $user->id)}}" method="post" style="display: inline-block">
        @csrf
        @method('GET')
        <button class="btn btn-success btn-sm" type="submit"><i class="ti-lock"></i>Change Password</button>
        </form>
    </td>-->
     <td>
    <form action="{{ url('users/edit', $user->id)}}" method="post" style="display: inline-block">
